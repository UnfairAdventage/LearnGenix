...
const EditExercise: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setFormData({
        ...
        subject: 'mathematics',
        question: '¿Cuál es el resultado de resolver la ecuación x² - 5x + 6 = 0?',
        type: 'multiple-choice',
        difficulty: 'medium',
        correctAnswer: 'x = 2, x = 3',
        explanation: 'Para resolver esta ecuación cuadrática, podemos factorizar: (x - 2)(x - 3) = 0, por lo tanto x = 2 o x = 3.',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('Exercise updated:', formData);
  ...
  const handleDelete = () => {
    if (window.confirm('¿Estás seguro de que quieres eliminar este ejercicio? Esta acción no se puede deshacer.')) {
      // Here you would typically delete the exercise via your backend
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-text-primary">Editar ejercicio</h1>
              <p className="text-text-secondary text-lg">Modifica este ejercicio para tus estudiantes</p>
            </div>
          </div>
          <button
            onClick={handleDelete}
            className="bg-pink-neon/10 border border-pink-neon/30 hover:bg-pink-neon/20 text-pink-neon py-2 px-4 rounded-xl font-semibold transition-all duration-300 flex items-center space-x-2"
          >
